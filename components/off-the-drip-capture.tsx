...
export function OffTheDripCapture({
  ...
  const shell =
    mode === "popup"
      ? "fixed inset-x-3 bottom-3 z-[1000001] mx-auto max-w-[520px] rounded-[18px] border p-4 shadow-[0_18px_70px_rgba(17,17,17,0.28)] sm:bottom-5 sm:right-5 sm:left-auto"
    ...
    <div className={`${shell} ${colors}`}>
      {mode === "popup" && (
        <button
          type="button"
          onClick={dismiss}
          className="absolute right-3 top-3 z-10 flex h-9 w-9 items-center justify-center rounded-full text-2xl leading-none text-current/70 transition hover:bg-current/10 hover:text-current"
