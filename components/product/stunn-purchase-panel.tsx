...
export function StunnPurchasePanel({ product }: { product: Product }) {
  ...
  return (
    <div className="px-6 py-8 lg:px-10">
      ...
      {/* Benefit chips — large, scannable */}
      <div className="mb-5 flex flex-wrap gap-2">
        {BENEFIT_CHIPS.map((chip) => (
          <span
            key={chip}
            className="rounded-full border border-[#5A3493]/30 bg-[#EDE9F8] px-3 py-1 font-[family-name:var(--font-anton)] text-sm uppercase tracking-wide text-[#5A3493]"
          >
            {chip}
          </span>
        ))}
      ...
      <div className="mb-1 flex items-baseline gap-3">
        <span className="text-3xl font-bold text-gray-900">${display.subPrice.toFixed(2)}</span>
        <span className="text-base text-gray-400 line-through">${display.retailPrice.toFixed(2)}</span>
        <span className="rounded-full bg-[#C1D0A5] px-2 py-0.5 text-xs font-bold text-green-900">
      ...
      <Accordion title="INGREDIENTS" defaultOpen>
        <p className="text-sm leading-relaxed text-gray-600">
          <strong className="text-gray-900">STUNN</strong> is crafted with a clean, functional blend of Lion&apos;s Mane, Rhodiola, Cordyceps, and L-Theanine to support focus, balance, and steady energy.
        </p>
      </Accordion>
      <Accordion title="WHY STUNN?">
        <p className="text-sm leading-relaxed text-gray-600">
          Most coffees give you energy and anxiety together. STUNN separates them — delivering the focus and ritual you love, without the spike, crash, or dependency.
        </p>
      </Accordion>
